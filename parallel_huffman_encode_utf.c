...
int main() {
    ...
    pid_t pid;
    int processCount = 0;
    //El numero de procesos maximos se inicializa segun el numero de nucleos logicos
    int maxProcesses = sysconf(_SC_NPROCESSORS_ONLN); // Ajustar esto al número de procesos que se desea usar
    printf("Numero de procesadores en linea: %ld\n", maxProcesses);
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            ...
            if (fileExtension && strcmp(fileExtension, ".txt") == 0 && strlen(fileExtension) == 4) {
                if (processCount >= maxProcesses) {
                    wait(NULL); // Espera a que termine un proceso antes de crear otro
                    processCount--;
                }
        ...
        // Llama a compressFiles para comprimir los archivos .huffman y .table
